...
if ($request['group']!='') {
    ...
    <tr>
        ...
        <td><p align='center' class='block'>
            ...
            <input type="image" name="<?=$key;?>" onmouseover="soundButton();" class="power" onclick="soundClick(); if (!isLocalhost()) {
            window.location.href='https://yandex.ru/images/search?rpt=imageview&url='+(this.name).replace(/^url\(['"]?(.*?)['"]?\)$/i,'$1');
        }" src="<?=$prefix[3].'world.webp';?>">
        </p></td>
