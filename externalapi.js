...
async function fetchGitLabContent(projectUrl) {
    try {
	const urlObj=new URL(projectUrl);
	let pathParts=urlObj.pathname.replace(/\.git$/i,'').split('/').filter(Boolean);
	if (pathParts.length<2) throw new Error('Invalid GitLab URL');
	const projectName=pathParts.pop();
	const namespace=pathParts.join('/');
	const encodedPath=encodeURIComponent(`${namespace}/${projectName}`);
	const apiBase=`${urlObj.origin}/api/v4/projects/${encodedPath}`;
	const response=await fetch(apiBase,{
	    ...
	    console.error(`GitLab API Error ${response.status}:`,errorData.message);
	    throw new Error(`API Error: ${response.status} - ${errorData.message}`);
	} const data=await response.json(); await sleep(500);
	return {
	    name: data.name, owner: data.namespace.full_path,
	    description: data.description, stars: data.star_count,
	    forks: data.forks_count, language: data.language,
	    visibility: data.visibility, lastActivity: data.last_activity_at
	};
    } catch (error) {
	console.error('Error fetching from GitLab:',error.message); return null;
    }
}
async function analyzeMultipleGitLabRepositories(repoUrls) {
    try {
	const allReposInfo=await Promise.all(
	    repoUrls.map(async(url)=>{
		const repoInfo=await fetchGitLabContent(url); if (repoInfo) {
		    return `Repository: ${repoInfo.name} Owner (Namespace): ${repoInfo.owner} Description: ${repoInfo.description||'No description'} Stars: ${repoInfo.stars} Forks: ${repoInfo.forks||0} Language: ${repoInfo.language||'Not specified'} Visibility: ${repoInfo.visibility} Last Activity: ${repoInfo.lastActivity}`;
		} return null;
	    })
	); const validRepos=allReposInfo.filter(info=>info!==null);
	return validRepos.join('\n\n---\n\n');
    } catch (error) {
	console.error('Error analyzing repositories batch:',error); return null;
    }
}
...
async function analyzeMultipleGitHubRepositories(repoUrls) {
    try {
        const allReposInfo=await Promise.all(
            repoUrls.map(async(url)=>{
...
async function AI(input) {
    try {
        ...
        if (input.includes('https://github.com/')) {
            ...
            const allReposInfo=await analyzeMultipleGitHubRepositories(repoUrls);
            userContent=createUserMessage(`${input}\n${allReposInfo}\n${envInfo}`,userContext);
	} else if (input.includes('https://gitlab.com/')) {
            const repoUrls=input.match(/https:\/\/gitlab\.com\/[^\s,.<>;"']+/g)||[];
            const allReposInfo=await analyzeMultipleGitLabRepositories(repoUrls);
            userContent=createUserMessage(`${input}\n${allReposInfo}\n${envInfo}`,userContext);
        } else {
            userContent=createUserMessage(`${input}\n${envInfo}`,userContext);
        ...
        if (authstate()) {
	    set(sysDefSessionID.value+'_files/artificial_intelligence.json',encodeURIComponent(JSON.stringify(historyArr)),'rw');
	} return reply;
