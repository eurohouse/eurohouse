...
<div class='customPanel' style="width:100%;height:1.7em;left:0px;top:0px;">
    ...
    <input type="text" id="promptGPT" style="width:54%;" placeholder="<?=term("Ask artificial intelligence anything",$settings,$session);?>" value="" onkeydown="if (event.keyCode==13) {
    ...
    <input type="image" class="power" id="buttonTrashBackspace" onmouseover="soundButton();" src="<?=$prefix[3].((authstate())?'trash.png':'backspace.png');?>" onclick="soundClick(); promptGPT.value=''; markdown_disp.innerHTML=''; promptGPT.focus();
if (authstate()) {
    set(sysDefSessionID.value+'_files/artificial_intelligence.json','[]','rw');
    sysDefUserAICurNum.value=0;
}">
