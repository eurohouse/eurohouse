...
<div class='customPanel' style="width:100%;height:15%;left:0px;top:0px;">
    ...
    } else if (event.keyCode==27) { promptGPT.value=''; promptGPT.focus();
    } else if (event.keyCode==8) { handleInput(this.value);
    } else if (event.keyCode==46) { handleInput(this.value);
    }" oninput="handleInput(this.value,true);">
    <input type="image" class="power" id="buttonKeyboard" onmouseover="soundButton();" src="<?=$prefix[3].'keyboard.png';?>" onclick="soundClick(); chatGPTAI(promptGPT.value).then(reply=>{
        if (notNull(reply)) {
            markdown_disp.innerHTML=marked.parse(reply);
        }
    }).catch(error=>{ console.error(error); });">
