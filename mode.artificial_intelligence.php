...
<div class='customPanel' style="width:100%;height:1.3em;left:0px;top:0px;">
	...
	<span id='AIInfo' class='urgent' name="<?=(isUserRoot($superuser))?'set "../.env" "API_KEY=..."':'endpoint: '.fileopen($superuser.'_files/profile.json',json_encode($settings['defaults']))['endpoint'].'; model: '.fileopen($superuser.'_files/profile.json',json_encode($settings['defaults']))['model'].';';?>" onclick="soundClick(true); if (authstate()) { clip(this.name); } else { executeCode(this.name); }">
