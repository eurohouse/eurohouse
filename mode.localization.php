...
<?php $arr=str_replace('.png','',(str_replace('Flag.','',str_replace('./','',(glob('./Flag.*.png')))))); foreach ($arr as $val) { ?>
    <img name="<?=$val;?>" style="height:17%;opacity:<?=(in_array($val,explode(',',$session['units_list'])))?1:0.5;?>;" title="<?=$val;?>" src="<?='Flag.'.$val.'.png';?>" onclick="soundClick(); setdata('units_list',arrangeMenu(sysDefUnitsList.value,this.name,',',true)); if (this.style.opacity==0.5) { this.style.setProperty('opacity',1); } else { this.style.setProperty('opacity',0.5); }">
<?php } ?>
</p>