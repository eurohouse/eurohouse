...
<html><head><meta name="viewport" content="<?=$viewportParam;?>"><meta charset="UTF-8">
...
<?php } foreach ($settings['libraries']['foreign'] as $val) { ?>
    <script src="<?=$val;?>"></script>
<?php } include 'frontend.php'; include 'startup.php'; ?></head>
<!-- Google tag (gtag.js) -->
...
</form><p align='center' class='block'>
<input hidden type="image" onmouseover="soundButton();" class="power" id="powerButton" onclick="setdata('observe', flip(sysDefObserve.value));" src="<?=$prefix.'power.png';?>">
</p><div class='topbar'>
...
</div><div class='upperGap'>
<span id='showUsUrgent' style="cursor:pointer;" onclick="navigator.clipboard.writeText(this.innerText);"></span><audio id="audioPlayer" style="width:80%;position:relative;" preservesPitch="<?=boolval($session['pitch_lock']);?>" volume="<?=$session['audio_volume'];?>" playbackRate="<?=$session['audio_speed'];?>" onended="if (sysDefAutoplay.value != 0) {
    songIndex(sysDefSongIndex.value);
}" ontimeupdate="savePlayState();" onpause="setdata('playing', 0); $('#buttonPlay').attr('src', sysDefPrefix.value+'play.png');" onplay="setdata('playing', 1); $('#buttonPlay').attr('src', sysDefPrefix.value+'pause.png');" onvolumechange="setdata('audio_volume', this.volume);" onratechange="setdata('audio_speed', this.playbackRate);">
</div><div class='panel'><?php
if (file_exists('mode.'.$request['mode'].'.php')) {
    include 'mode.'.$request['mode'].'.php';
} else {
    include 'welcome.php';
} ?></div><div class='lowerGap'><span id='showUsText' style="cursor:pointer;" onclick="navigator.clipboard.writeText(this.innerText);"></span></div>
...
<audio id="alarmPlayer" src="<?=$session['alarm_sound'];?>" volume="<?=$session['alarm_volume'];?>" onended="$('#buttonAlarm').attr('src', sysDefPrefix.value+'call.png');" onplay="$('#buttonAlarm').attr('src', sysDefPrefix.value+'dial.png');" onpause="$('#buttonAlarm').attr('src', sysDefPrefix.value+'call.png');">
