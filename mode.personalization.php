<!-- speed -->
<!-- AD: Personalización; AG: Personalización; AR: Personalización; AT: Personalisierung; BE: Personnalisation; BR: Personalização; CH: Occasus Personalis; CL: Personalización; CN: 个性化和元数据; CO: Personalización; CU: Personalización; CY: Προσωπικότητα; DD: Personalisierung; DE: Personalisierung; DR: Personalisierung; ES: Personalización; FR: Personnalisation; GR: Προσωπικότητα; IN: वैयक्तिकरण; IT: Personalizzazione; JP: カスタマイズする; KP: 개인화하다; KR: 개인화하다; LK: व्यक्तिगतकरणम्; MC: Personnalisation; MD: Personalizare; MX: Personalización; NP: མི་སྒེར་ཅན་བཟོ་བ།; PT: Personalização; RO: Personalizare; RS: Персонализација; RU: Персонализация; SM: Personalizzazione; SP: Occasus Personalis; TR: Kişiselleştirme; UA: Персоналізація; VA: Occasus Personalis -->
<div class='customPanel' id='prefs_btns' style="width:100%;height:60px;left:0px;top:0px;">
    <p align='center'>
        <input type='button' onmouseover="soundButton();" value="<?=term('Apply Settings',$settings,$session)?>" onclick="setdata('box_shadow',setBoxShadow.value); setdata('text_box_shadow',setTextBoxShadow.value); setdata('marquee',setMarqueeSpeed.value); setdata('mediainfo',setMediainfoSpeed.value); setdata('border_radius',setBorderRadius.value); setdata('text_border_radius',setTextBorderRadius.value); setdata('gradient_fore',setGradFore.value); setdata('gradient_input',setGradInput.value); setdata('gradient_button',setGradButton.value); setdata('magnitude',setMagnitude.value); setdata('specimen',encodeURIComponent(setSpecimen.value)); setdata('background',setBackImage.value); setdata('back_size',setBackSize.value); setdata('fore_size',setForeSize.value); setdata('input_size',setInputSize.value); setdata('head1_size',setHead1Size.value); setdata('head2_size',setHead2Size.value); setdata('head3_size',setHead3Size.value); setdata('disp_size',setDispSize.value); setdata('back_color',setBackColor.value); setdata('fore_color',setForeColor.value); setdata('input_color',setInputColor.value); setdata('back_text_color',setBackTextColor.value); setdata('fore_text_color',setForeTextColor.value); setdata('input_text_color',setInputTextColor.value); setdata('blank_color',setBlankColor.value); setdata('blank_text_color',setBlankTextColor.value); setdata('arc_fore_color',setArcForeColor.value); setdata('arc_input_color',setArcInputColor.value);">
...
<div class='customPanel' id='prefs_tab' style="width:100%;height:70%;left:0px;top:0px;overflow-y:scroll;">
    <p align='center'>
        ...
        <input type="text" id="setSpecimen" style="width:68%;" placeholder="<?=term('Specimen Text',$settings,$session);?>" value="<?=$session['specimen'];?>" onkeydown="if (event.keyCode==13) {
            ...
            this.value='QfSzGh ÆøÐñÇê ʒʊʎɸʃŋ ΣφΩθΞψ ЭшГжУя عليخالد 인방학적괴굠 のあぱオヰカ 中英რՖטभो 🍷☕️🍾🍫🍍🌴';
        ...
        <input type="number" min='0' max='1000' step='50' id="setMarqueeSpeed" style="width:38%;" placeholder="<?=term('Marquee Speed',$settings,$session);?>" value="<?=$session['marquee'];?>" onkeydown="if (event.keyCode==27) {
            this.value='1000';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('marquee',this.value); handleInput(this.value,true);">
        <input type="number" min='18' max='26' step='2' id="setDispSize" style="width:38%;" placeholder="<?=term('Display Text Size',$settings,$session);?>" value="<?=$session['disp_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='20';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('disp_size',this.value); handleInput(this.value,true);"><br>
        <input type="number" min='0' max='270' step='90' id="setGradButton" style="width:26%;" placeholder="<?=term('Button Gradient',$settings,$session);?>" value="<?=$session['gradient_button'];?>" onkeydown="if (event.keyCode==27) {
            this.value='180';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('gradient_button',this.value); handleInput(this.value,true);">
        <input type="number" min='0' max='270' step='90' id="setGradFore" style="width:26%;" placeholder="<?=term('Fore Gradient',$settings,$session);?>" value="<?=$session['gradient_fore'];?>" onkeydown="if (event.keyCode==27) {
            this.value='180';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('gradient_fore',this.value); handleInput(this.value,true);">
        <input type="number" min='0' max='270' step='90' id="setGradInput" style="width:26%;" placeholder="<?=term('Input Gradient',$settings,$session);?>" value="<?=$session['gradient_input'];?>" onkeydown="if (event.keyCode==27) {
            this.value='180';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('gradient_input',this.value); handleInput(this.value,true);"><br>
        <input type="number" min='8' max='16' step='2' id="setBackSize" style="width:26%;" placeholder="<?=term('Back Size',$settings,$session);?>" value="<?=$session['back_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='16';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('back_size',this.value); handleInput(this.value,true);">
        <input type="number" min='8' max='16' step='2' id="setForeSize" style="width:26%;" placeholder="<?=term('Fore Size',$settings,$session);?>" value="<?=$session['fore_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='16';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('fore_size',this.value); handleInput(this.value,true);">
        <input type="number" min='8' max='16' step='2' id="setInputSize" style="width:26%;" placeholder="<?=term('Input Size',$settings,$session);?>" value="<?=$session['input_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='16';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('input_size',this.value); handleInput(this.value,true);"><br>
        <input type="number" min='16' max='20' step='1' id="setHead1Size" style="width:26%;" placeholder="<?=term('Heading Level 1 Size',$settings,$session);?>" value="<?=$session['head1_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='19';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('head1_size',this.value); handleInput(this.value,true);">
        <input type="number" min='16' max='20' step='1' id="setHead2Size" style="width:26%;" placeholder="<?=term('Heading Level 2 Size',$settings,$session);?>" value="<?=$session['head2_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='18';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('head2_size',this.value); handleInput(this.value,true);">
        <input type="number" min='16' max='20' step='1' id="setHead3Size" style="width:26%;" placeholder="<?=term('Heading Level 3 Size',$settings,$session);?>" value="<?=$session['head3_size'];?>" onkeydown="if (event.keyCode==27) {
            this.value='17';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="setdata('head3_size',this.value); handleInput(this.value,true);"><br>
        <input type="text" id="setBackColor" style="width:38%;" placeholder="<?=term('Back Color',$settings,$session);?>" value="<?=$session['back_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('back_color',this.value);
        } else if (event.keyCode==27) { this.value='C0BFC0';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);">
        <input type="text" id="setForeColor" style="width:38%;" placeholder="<?=term('Fore Color',$settings,$session);?>" value="<?=$session['fore_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('fore_color',this.value);
        } else if (event.keyCode==27) { this.value='605F60';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);"><br>
        <input type="text" id="setInputColor" style="width:38%;" placeholder="<?=term('Input Color',$settings,$session);?>" value="<?=$session['input_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('input_color',this.value);
        } else if (event.keyCode==27) { this.value='E5E5E5';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);">
        <input type="text" id="setArcForeColor" style="width:38%;" placeholder="<?=term('Adverse Fore Color',$settings,$session);?>" value="<?=$session['arc_fore_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('arc_fore_color',this.value);
        } else if (event.keyCode==27) { this.value='403F40';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);"><br>
        <input type="text" id="setArcInputColor" style="width:38%;" placeholder="<?=term('Adverse Input Color',$settings,$session);?>" value="<?=$session['arc_input_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('arc_input_color',this.value);
        } else if (event.keyCode==27) { this.value='D5D5D5';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);">
        <input type="text" id="setBackTextColor" style="width:38%;" placeholder="<?=term('Back Text Color',$settings,$session);?>" value="<?=$session['back_text_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('back_text_color',this.value);
        } else if (event.keyCode==27) { this.value='FFFFFF';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);"><br>
        <input type="text" id="setForeTextColor" style="width:38%;" placeholder="<?=term('Fore Text Color',$settings,$session);?>" value="<?=$session['fore_text_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('fore_text_color',this.value);
        } else if (event.keyCode==27) { this.value='FFFFFF';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);">
        <input type="text" id="setInputTextColor" style="width:38%;" placeholder="<?=term('Input Text Color',$settings,$session);?>" value="<?=$session['input_text_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('input_text_color',this.value);
        } else if (event.keyCode==27) { this.value='000000';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);"><br>
        <input type="text" id="setBlankColor" style="width:38%;" placeholder="<?=term('Blank Color',$settings,$session);?>" value="<?=$session['blank_color'];?>" onkeydown="if (event.keyCode==13) {
            setdata('blank_color',this.value);
        } else if (event.keyCode==27) { this.value='FFFFFF';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        }" oninput="handleInput(this.value,true);">
        <input type="text" id="setBlankTextColor" style="width:38%;" placeholder="<?=term('Blank Text Color',$settings,$session);?>" value="<?=$session['blank_text_color'];?>" onkeydown="if (event.keyCode==13) {
