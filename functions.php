...
function visitor($username='') {
    $vis=fileopen('visitors.json','{}','create');
    if ($username!='') {
...
function modelcard($id,$cont,$exem,$ses,$sti) {
    ...
    if (isset($exem[$id])) {
        ...
        $len=(isset($ent['height']))?((isset($loc['length'][$uni]))?((isset($loc['length'][$uni]['inch']))?incher($ent['height']):round(($ent['height']*$loc['length'][$uni]['coefficient']),2).' '.$loc['length'][$uni]['sign']):round(($ent['height']*$loc['length']['default']['coefficient']),2).' '.$loc['length']['default']['sign']):'0 '.((isset($loc['length'][$uni]))?$loc['length'][$uni]['sign']:$loc['length']['default']['sign']);
        $mas=(isset($ent['weight']))?((isset($loc['mass'][$uni]))?(round($ent['weight']*$loc['mass'][$uni]['coefficient']).' '.$loc['mass'][$uni]['sign']):round($ent['weight']*$loc['mass']['default']['coefficient']).' '.$loc['mass']['default']['sign']):'0 '.((isset($loc['mass'][$uni]))?$loc['mass'][$uni]['sign']:$loc['mass']['default']['sign']);
        $bod=(isset($ent['sizes']))?((isset($loc['length'][$uni]['inch']))?(round(explode('-',$ent['sizes'])[0]*$loc['length'][$uni]['coefficient']).'-'.round(explode('-',$ent['sizes'])[1]*$loc['length'][$uni]['coefficient']).'-'.round(explode('-',$ent['sizes'])[2]*$loc['length'][$uni]['coefficient'])):(explode('-',$ent['sizes'])[0].'-'.explode('-',$ent['sizes'])[1].'-'.explode('-',$ent['sizes'])[2])):'0-0-0';
        $sho=(isset($ent['shoe_size']))?((isset($loc['shoe_size'][$uni]))?(($ent['shoe_size']+$loc['shoe_size'][$uni]).' '.$uni):($ent['shoe_size']+$loc['shoe_size']['default']).' '.$uni):'0 '.$uni;
        $bday=(isset($ent['birthday']))?$ent['birthday']:"now";
        $dday=(isset($ent['deathday']))?$ent['deathday']:"now";
        $today=date_create("now"); $bdT=date_create($bday); $ddT=date_create($dday);
        $annoDiff=date_diff($bdT,$ddT)->format('%y');
        $nextBday=new DateTime();
        $nextBday->setDate($today->format('Y'),$bdT->format('m'),$bdT->format('d'));
        if ($nextBday<$today) { $nextBday->modify('+1 year'); }
        $interval=$today->diff($nextBday); $daysDiff=$interval->days;
        ...
        $titleT=((isset($ent['birthday']))?$zodT.' '.$title.$cakeT:$title); $res=[
            'title'=>$titleT,'text'=>$text,
            'url'=>$url,'avatar'=>$ava,
