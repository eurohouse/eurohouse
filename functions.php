...
function catlist($cat): array { return str_replace('./','',(glob('./'.$cat.'.*.00.png'))); }
...
function escHTML($str) { return str_replace(' -->','',str_replace('<!-- ','',$str)); }
...
function offnum($pos=0,$all=360,$off=90) { return abs(($pos+$all-abs($off))%$all); }
...
function horizontal($a,$circle=6.285714285714286) { return floor(4*($a/$circle)); }
...
function wasAuth() {
    if ((isset($_POST['auth']))&&(isset($_POST['login']))&&(isset($_POST['password']))) {
        ...
        if ($auth=='signup') {
            ...
            if ($pass==$logpass) { $_SESSION['user']=$login; }
        } elseif ($auth=='signout') { unset($_SESSION['user']); }
    }
}
function isAuth() {
...
function wordfx($word,$sup,array $voc,array $ses) {
    ...
    for ($i=0; $i<count($matches[0]); $i++) {
        $full=$matches[0][$i]; switch ($full) {
            ...
            case '[df]': $res=sizestr(disk_free_space('/'),$voc['locale'],$ses['units']); break;
            case '[french]':
