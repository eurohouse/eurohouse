...
function calculate(expr) {
    ...
    } else if (/(\d*)([A-Z][a-z]*\d*)/g.test(expr)) {
...
function populateCommandIO() {
    if (requestMode.value==='terminal') {
        ...
        try {
            if (((promptExec.value).match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\:\:)$/gi))||((promptExec.value).match(/^([\w\s\u0400-\u04FF]{2,})(?:\:\:)([\d|\*]{1,})$/gi))) {
                row.insertCell().textContent=seekImage(promptExec.value);
            } else if (((promptExec.value).match(/([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\:)$/gi))||((promptExec.value).match(/([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\:)([\w|\s|\*]{2,})$/gi))) {
                row.insertCell().textContent=seekCode(promptExec.value);
            } else if (((promptExec.value).match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\?)$/gi))||((promptExec.value).match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\?)([\d|\*|\+]{1,})$/gi))||((promptExec.value).match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\?)([\d|\*|\+]{1,})(?:\:)([-?\d(\.\d)?|\*|\+]{1,})$/gi))) {
                row.insertCell().textContent=seekMusic(promptExec.value);
            } else if ((promptExec.value).endsWith(';')) {
                row.insertCell().textContent=executeCode(promptExec.value);
            } else {
...
class MorseSequentialPlayer {
    constructor(speedWPM=20,frequencyHz=600) {
        this.audioCtx=null; this.wpm=speedWPM;
        this.freq=frequencyHz; this.isPlaying=false;
        this.dotLengthMs=1.2/this.wpm*1000;
    }
    init() {
        if (!this.audioCtx) {
    ...
    _playTone(durationMs) {
        if (!this.audioCtx||this.audioCtx.state==='closed') {
            return Promise.resolve();
        } return new Promise(resolve=>{
            const osc=this.audioCtx.createOscillator();
            const gainNode=this.audioCtx.createGain(); osc.type='sine';
            osc.frequency.setValueAtTime(this.freq,this.audioCtx.currentTime);
            gainNode.gain.setValueAtTime(0,this.audioCtx.currentTime);
            gainNode.gain.linearRampToValueAtTime(1,this.audioCtx.currentTime+0.005);
            gainNode.gain.setValueAtTime(1,this.audioCtx.currentTime+durationMs/1000-0.005);
            ...
            osc.start(); setTimeout(()=>{
                osc.stop(); osc.disconnect(); resolve();
            },durationMs);
        });
    }
    async _playFragment(fragment) {
        const trimmed=fragment.trim();
        for (let i=0; i<trimmed.length; i++) {
            if (!this.isPlaying) return;
            const char=trimmed[i];
            if (char==='.'||char==='-') {
                const duration=char==='.'?this.dotLengthMs:this.dotLengthMs*3;
                await this._playTone(duration);
                if (i<trimmed.length-1) {
                    await this._delay(this.dotLengthMs);
                }
            } else if (char===' ') {
                await this._delay(this.dotLengthMs*3);
            } __loopProtector();
        }
    }
    async playFullSequence(fullText) {
        if (this.isPlaying) return;
        this.init(); if (this.audioCtx.state==='suspended') {
            await this.audioCtx.resume();
        } this.isPlaying=true; const fragments=fullText.split('/');
        for (let i=0; i<fragments.length; i++) {
            if (!this.isPlaying) return;
            const currentFragment=fragments[i];
            await this._playFragment(currentFragment);
            if (i<fragments.length-1) {
                await this._delay(this.dotLengthMs*7);
            } __loopProtector();
        } this.isPlaying=false;
    }
} const sequentialPlayer=new MorseSequentialPlayer(20,650);