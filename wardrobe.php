...
:root {
    ...
    --background: url("<?=$background;?>?rev=<?=time();?>");
    ...
    --box-shadow: <?=$session['box_shadow'];?>px <?=$session['box_shadow'];?>px <?=$session['box_shadow'];?>px black;
...
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_ascii'];?>?rev=<?=time();?>");
    unicode-range: U+0000-007F;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_latin'];?>?rev=<?=time();?>");
...
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_greek'];?>?rev=<?=time();?>");
    unicode-range: U+0370-03FF;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_cyril'];?>?rev=<?=time();?>");
    unicode-range: U+0400-04FF;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_arabi'];?>?rev=<?=time();?>");
    unicode-range: U+0590-077F;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_korea'];?>?rev=<?=time();?>");
    unicode-range: U+1100-11FF, U+3130-318F, U+A000-D7A3;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_china'];?>?rev=<?=time();?>");
    unicode-range: U+2E80-312F, U+3190-9FFF, U+D7A4-FFFF;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_other'];?>?rev=<?=time();?>");
    unicode-range: U+0780-10FF, U+1200-25FF, U+2700-2E7F, U+10000-1F2FF, U+1F650-1F67F, U+1F700-10FFFF;
}
@font-face {
    font-family: "euro";
    src: url("<?=$session['font_emoji'];?>?rev=<?=time();?>");
    unicode-range: U+2600-26FF, U+1F300-1F64F, U+1F680-1F6FF;
}
@font-face {
    font-family: "userDefine";
    src: url("<?=$request['input'];?>?rev=<?=time();?>");
}
body {
    ...
    cursor: url("<?=$session['cursor_auto'];?>?rev=<?=time();?>"), auto;
    cursor: url("<?=$session['cursor_default'];?>?rev=<?=time();?>"), default;
    cursor: url("<?=$session['cursor_none'];?>?rev=<?=time();?>"), none;
    cursor: url("<?=$session['cursor_context_menu'];?>?rev=<?=time();?>"), context-menu;
    cursor: url("<?=$session['cursor_help'];?>?rev=<?=time();?>"), help;
    cursor: url("<?=$session['cursor_pointer'];?>?rev=<?=time();?>"), pointer;
    cursor: url("<?=$session['cursor_progress'];?>?rev=<?=time();?>"), progress;
    cursor: url("<?=$session['cursor_wait'];?>?rev=<?=time();?>"), wait;
    cursor: url("<?=$session['cursor_cell'];?>?rev=<?=time();?>"), cell;
    cursor: url("<?=$session['cursor_crosshair'];?>?rev=<?=time();?>"), crosshair;
    cursor: url("<?=$session['cursor_text'];?>?rev=<?=time();?>"), text;
    cursor: url("<?=$session['cursor_vertical_text'];?>?rev=<?=time();?>"), vertical-text;
    cursor: url("<?=$session['cursor_alias'];?>?rev=<?=time();?>"), alias;
    cursor: url("<?=$session['cursor_copy'];?>?rev=<?=time();?>"), copy;
    cursor: url("<?=$session['cursor_move'];?>?rev=<?=time();?>"), move;
    cursor: url("<?=$session['cursor_no_drop'];?>?rev=<?=time();?>"), no-drop;
    cursor: url("<?=$session['cursor_not_allowed'];?>?rev=<?=time();?>"), not-allowed;
    cursor: url("<?=$session['cursor_all_scroll'];?>?rev=<?=time();?>"), all-scroll;
    cursor: url("<?=$session['cursor_col_resize'];?>?rev=<?=time();?>"), col-resize;
    cursor: url("<?=$session['cursor_row_resize'];?>?rev=<?=time();?>"), row-resize;
    cursor: url("<?=$session['cursor_n_resize'];?>?rev=<?=time();?>"), n-resize;
    cursor: url("<?=$session['cursor_s_resize'];?>?rev=<?=time();?>"), s-resize;
    cursor: url("<?=$session['cursor_e_resize'];?>?rev=<?=time();?>"), e-resize;
    cursor: url("<?=$session['cursor_w_resize'];?>?rev=<?=time();?>"), w-resize;
    cursor: url("<?=$session['cursor_ns_resize'];?>?rev=<?=time();?>"), ns-resize;
    cursor: url("<?=$session['cursor_ew_resize'];?>?rev=<?=time();?>"), ew-resize;
    cursor: url("<?=$session['cursor_ne_resize'];?>?rev=<?=time();?>"), ne-resize;
    cursor: url("<?=$session['cursor_nw_resize'];?>?rev=<?=time();?>"), nw-resize;
    cursor: url("<?=$session['cursor_se_resize'];?>?rev=<?=time();?>"), se-resize;
    cursor: url("<?=$session['cursor_sw_resize'];?>?rev=<?=time();?>"), sw-resize;
    cursor: url("<?=$session['cursor_nesw_resize'];?>?rev=<?=time();?>"), nesw-resize;
    cursor: url("<?=$session['cursor_nwse_resize'];?>?rev=<?=time();?>"), nwse-resize;
    cursor: url("<?=$session['cursor_zoom_in'];?>?rev=<?=time();?>"), zoom-in;
    cursor: url("<?=$session['cursor_zoom_out'];?>?rev=<?=time();?>"), zoom-out;
}
.topBarItem {
...
.topbar {
    ...
    width: 100%; height: 280px; left: 0%; top: -2px;
}
.upperGap {
    border: none; border-radius: 0px; position: relative;
    background-color: var(--bicolor); color: var(--backtextcolor);
    width: 100%; height: 40px; left: 0%; top: -2px;
    text-align: center; overflow-y: scroll;
}
.panel {
    border: none; border-radius: 0px; position: relative;
    background-color: var(--bicolor); color: var(--backtextcolor);
    overflow-y: scroll; width: 100%; height: 52%; left: 0%; top: -2px;
}
.lowerGap {
    border: none; border-radius: 0px; position: relative;
    background-color: var(--bicolor); color: var(--backtextcolor);
    width: 100%; height: 130px; left: 0%; top: -2px;
    text-align: center; overflow-y: scroll;
}
.customPanel {
    border: none; border-radius: 0px; position: relative;
...
input[type=button], input[type=image], marquee, button {
    ...
    background-size: 100%; color: var(--foretextcolor);
    border: none; border-radius: var(--radius);
    font-family: "euro"; font-size: var(--foresize);
    position: relative; height: 1.4em; box-shadow: var(--box-shadow);
}
input[type=text], input[type=password], input[type=number], select, textarea {
    background: var(--gradient-input);
    background-size: 100%; color: var(--inputtextcolor);
    border: none; border-radius: var(--radius);
    font-family: "euro"; font-size: var(--inputsize);
    position: relative; height: 1.4em; box-shadow: var(--box-shadow);
}
input[type=checkbox] { width: 1.4em; height: 1.4em; position: relative; box-shadow: var(--box-shadow); }
...
.slider {
    ...
    appearance: none; width: 40%; height: 26px;
    border-radius: var(--radius); background: var(--backcolor);
...
.slider::-webkit-slider-thumb {
    ...
    background: var(--forecolor); cursor: pointer;
}
