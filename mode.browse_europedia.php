...
<tbody><?php foreach ($exemplarArr as $key=>$value) {
    ...
    <tr>
        ...
        <?php } else { ?>
            <td><?php $bday=(isset($value['birthday']))?timedate(strtotime($value['birthday']),$session,$settings,'date'):''; /*$bday=(isset($value['birthday']))?chooseCalendar(strtotime($value['birthday']),$session,$settings):'';*/ echo $bday; ?></td>
