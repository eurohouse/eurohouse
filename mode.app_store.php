...
<p align='center'>
    <a href="javascript:getPkgSequence('get -i '+updateChannel<?=md5($key);?>.value, 'get ', 0);">
        <?=$key;?>
    </a><br>
<?php foreach ($val as $ch) {
    $pkgID = explode('/', $ch)[count(explode('/', $ch))-1]; ?>
    <input type="button" name="<?=$ch;?>" value="<?=$ch;?>" style="width:<?=$line1Size;?>%;" onmouseover="soundButton();" onclick="getPkgSequence('get -i '+this.name, 'get ');">
    <?php if (file_exists($pkgID.'.pkg')) { ?>
