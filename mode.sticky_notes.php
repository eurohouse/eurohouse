...
function openNote(id) {
    ...
    var cd=sysDefNumeric.value; myNotesDoc.value=hex2bin(metadata()[bin2hex(id,'',cd)],ci,cd);
    countNote();
}
function saveNote(id) {
    var ci=obfstr(CryptoJS.SHA256(myNotesEnc.value));
    var cd=myNotesRad.value; setmeta(bin2hex(id,'',cd),bin2hex(myNotesDoc.value,ci,cd));
}
