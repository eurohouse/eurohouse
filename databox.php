...
$bindingData=fileopen('binding.json',json_encode([$cookie=>$cookie]));
$poweredData=fileopen('dominion.json',json_encode([$cookie=>0]));
$autoData=fileopen('automator.json',json_encode([$cookie=>'manual']));
$toolData=fileopen('toolbox.json',json_encode([$sessionID=>'']));
...
$publicUserData=[]; foreach ($poweredData as $key=>$value) {
    if (file_exists($key.'_files/session.json')) {
        $testArr=fileopen($key.'_files/session.json',json_encode($userSettings['defaults']),'backup');
