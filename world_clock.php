...
if ($userData['calendar']=='Julian') {
    $dateStr=timedate($userData['date_format'],$userSettings,$userData['timezone'],(intval(date('Y')/100)-7));
} elseif ($userData['calendar']=='French') {
    $dateStr=french($userSettings,$userData['units']);
} else {
    $dateStr=timedate($userData['date_format'],$userSettings,$userData['timezone']);
} $timeStr=timedate($userData['time_format'],$userSettings,$userData['timezone']);
