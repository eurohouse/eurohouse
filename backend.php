...
include 'functions.php'; $viewportStr = ''; $settings = fileopen('settings.json');
...
$portfolioPrefix = (($themePrefix == 'iec') || ($themePrefix == 'iso')) ? 'org.' : ((themed($themePrefix, 'head,left0,left90,left180,left270,right0,right90,right180,right270')) ? $themePrefix : 'org.'); $degKoeff = (isset($settings['locale']['angle'][$units]['coefficient'])) ? $settings['locale']['angle'][$units]['coefficient'] : $settings['locale']['angle']['default']['coefficient'];
...
$lockMusic = excpkg($musicBox, $locks['music']);
$lockSounds = excpkg($orpheus, $locks['sound']);
natcasesort($lockMusic); array_unique($lockMusic);
natcasesort($lockSounds); array_unique($lockSounds);
$codexBox = str_replace('./','',(glob('./*.mac'))); $speechBox = str_replace('./','',(glob('./*.pro')));
$thematic = str_replace('./','',(glob('./*.start.png')));
$allUsers = str_replace('./','',(glob('./*_session.json')));
$lockFonts = excpkg($homer, $locks['font']);