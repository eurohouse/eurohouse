...
<input class="text" id="filename" name="<?=$request['mode'];?>" style="width:50%;" type="text" value="<?=$request['input'];?>" onkeydown="if (event.keyCode==13) {
    ...
    if ((sysDefSessionID.value=='root')&&(this.value!='')) {
        saveGUI();
    } else {
        omniRead(this.name,filename.value,'false');countText();
    }
} else if (event.keyCode==27) {
    filename.focus(); this.value='';
...
<input type="image" id="textEdRep" onmouseover="soundButton();" class="power" onclick="replaceText(findbox.value);countText();" src="<?=$prefix.'text.png';?>">
<input type="image" id="textEdRepAll" onmouseover="soundButton();" class="power" onclick="replaceTextAll(findbox.value);countText();" src="<?=$prefix.'copy.png';?>">
