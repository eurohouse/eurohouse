...
function readPlaylist() {
    var dec=atob(sysDefPlaylist.value);
    var arr=art=[]; if (dec.includes(' | ')) {
        art=dec.toString().split(' | ');
        for (idx in art) { arr.push(art[idx]); }
    } else if (dec=='') { art=dec.toString(); arr=[];
    } else { art=dec.toString(); arr.push(art);
    } return arr;
}
function playlistNext(name) {
    var dec=readPlaylist().join(' | ');
    var art=arrangeMenu(dec,name,' | ');
    return btoa(art);
}
...
function omniPlaylist(mode=0) {
    ...
    var next=(playlist.includes(' | '))?playlist.split(' | '):((playlist!='')?[playlist]:[]);
