...
function playlistCollectionHTML() {
    if (requestMode.value=='album_tracklist') {
        var playlist=readPlaylist(); var line="";
        plCol=((isMobileUserAgent())?1:sysDefPlaylistColumns.value);
        for (idx in playlist) {
            line+="<a href='javascript:setdata(%22playlist%22,playlistNext(%22"+rfc3986(playlist[idx])+"%22));'>"+(parseInt(idx)+1)+'. '+playlist[idx]+"</a><br>";
...
function albumCollectionHTML() {
	...
	var avatars=indexAvatars(sysDefAlbum.value);
        ...
        var albCol=((((sysDefAlbum.value=='avatar')||(sysDefAlbum.value=='pictogram'))||(isMobileUserAgent()))?1:sysDefAlbumColumns.value); if (sysDefAlbum.value=='music') {
            for (idx in album) {
                line+="<a href='javascript:setdata(%22playlist%22,playlistNext(%22"+rfc3986(album[idx])+"%22));'>"+(parseInt(idx)+1)+'. '+album[idx]+"</a><br>";
            }
        } else if (sysDefAlbum.value=='sound') {
            for (idx in album) {
                line+="<a href='javascript:omniListen(%22"+rfc3986(album[idx])+"%22,true);'>"+album[idx]+"</a><br>";
            }
        } else if (sysDefAlbum.value=='font') {
            for (idx in album) {
                line+="<a href='javascript:omniRead(%22font_book%22,%22"+rfc3986(album[idx])+"%22,%22"+requestLock.value+"%22);'>"+album[idx]+"</a><br>";
            }
