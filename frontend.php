...
class DataAutoLoader {
    forProfile(id) {
        if (id==sysDefSessionID.value) {
            ...
            omniListen(utob(arr1['melody']),false,parseFloat(arr1['current']));
        }
    }
    forSubscripion(id) {
        if (id==sysDefSessionID.value) {
...
function readPlaylist() {
    var text=utob(sysDefPlaylist.value);
...
function playlistNext(name) {
    ...
    return btou(arr.map(item=>`<${item}>`).join(' | '));
}
function populateIpStats(req='') {
    if (requestMode.value=='statistics') {
        ...
        if (req!='') {
            if (req.toString().startsWith('remote_addr ')) {
                seq=req.toString().replace('remote_addr ','');
                for (const ipElem in ipTab) {
                    if (seq.toString()!=ipTab[ipElem]['remote_addr']) {
                        delete ipTab[ipElem];
                    }
                }
            } else if (req.toString().startsWith('server_addr ')) {
                seq=req.toString().replace('server_addr ','');
                for (const ipElem in ipTab) {
                    if (seq.toString()!=ipTab[ipElem]['server_addr']) {
                        delete ipTab[ipElem];
                    }
                }
            } else if (req.toString().startsWith('system ')) {
...
function omniPlaylist(mode=0) {
    ...
    var playlist=utob(sysDefPlaylist.value);
    var next=readPlaylist();
    var melody=utob(sysDefMelody.value);
