...
function rename_user(id,to,pass=null) {
	...
	init_user(to,pass);
        ...
        copy(id+'_files/messenger.json',to+'_files/messenger.json','rw');
...
function readPlaylist() {
    var text=demorse(sysDefPlaylist.value,sysDefSessionID.value,sysDefNumeric.value);
...
function playlistNext(name) {
    ...
    return enmorse((arr.map(item=>`"${item}"`).join(',')),sysDefSessionID.value,sysDefNumeric.value);
}
function populateIpStats(req='') {
	...
	tableElem.setAttribute('class','wrapper'); tableBody.innerHTML='';
	    ...
	    row.insertCell().textContent=ipTab[ipElem]['server_addr'];
...
function processMessage(arr,sen,rec,msg='',index=0) {
    arr[enmorse(localizedTitle(sen)+' (@'+sen+') · '+toIso8601(Date.now()+index*1000)+' UTC',rec)]=enmorse(replaceLineBreaks(msg),rec); return arr;
}
...
function omniPlaylist(mode=0) {
    ...
    var playlist=demorse(sysDefPlaylist.value,sysDefSessionID.value,sysDefNumeric.value);
    var next=readPlaylist();
    var melody=demorse(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value);
