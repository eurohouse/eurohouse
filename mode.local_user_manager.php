...
foreach ($relBind as $key=>$value) {
    ...
    $powersInd = (is_numeric($powersData[$value])) ? $powersData[$value] : 0; ?><p align='center' class='block'>
    <input type="image" onmouseover="soundButton();" class="power" src="<?=$cookieLeftAva;?>"><input type="button" value="<?=($value == 'root') ? $value.'@'.$websiteID.':'.$request['path'].'#' : $value.'@'.$websiteID.':'.$request['path'].'$';?>" style="width:<?=$line1Size;?>%;" onmouseover="soundButton();">
