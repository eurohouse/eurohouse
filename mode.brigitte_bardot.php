...
<p align="center" class="block">
    ...
    <?php foreach ($settings['angles'] as $key=>$value) { ?>
        <option id="<?=$value;?>" <?php if ($request['angle']==$value) { ?> selected <?php } ?>>
            <?=$degPreSign.($value*$degKoeff).$degSign;?>
        </option>
    <?php } ?>
    </select>
    <input type="image" onmouseover="soundButton();" class="power" onclick="this.src = sysDefPrefix.value+((sysDefBardot.value!=0)?'plus':'min')+'.png'; setdata('bardot', flip(sysDefBardot.value), true);" src="<?=$prefix[3].(($session['bardot']!=0)?'min':'plus').'.png';?>">
</p>
<p align="center">
