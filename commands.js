...
function omniEnter() {
    ...
    } else if ((input.match(/([\w|\s]{2,})(?:\:)$/gi))||
    (input.match(/([\w|\s]{2,})(?:\:)([\w|\s|\*]{2,})$/gi))) {
        seekCode(input);
    } else if (input.match(/^[\-|\+](\d+\.?\d*|\.\d+)[a-z]$/g)) {
        if (input.match(/[c|e|j|n|t|q|u|v|w|x]$/)) {
            if (input.startsWith('+')) {
                arb=parseInt(input.slice(1,-1));
                setdata('memo',(parseInt(Date.now()/1000)+parseInt(arb)));
            } else if (input.startsWith('-')) {
                arb=parseInt(input.slice(0,-1));
                setdata('memo',(parseInt(Date.now()/1000)-parseInt(arb)));
            }
        } else if (input.match(/[d|f|g|i|m|o|s|y|z]$/)) {
            if (input.startsWith('+')) {
                arb=parseInt(input.slice(1,-1)); setdata('slideshow',arb);
            } else if (input.startsWith('-')) {
                arb=parseInt(input.slice(0,-1)); setdata('slideshow',arb);
            }
        } else if (input.match(/[a|b|k|l|p|r]$/)) {
            if (input.startsWith('+')) {
                arb=parseInt(input.slice(1,-1));
            } else if (input.startsWith('-')) {
                arb=parseInt(input.slice(0,-1));
            } omniListen(utob(sysDefMelody.value),false,arb);
        }
...
function executeMacros(input) {
    ...
    if (input.startsWith('# ')) { /* SAY SOMETHING... */
        ...
        } else if ((keyVal[0]=='playlist')||(keyVal[0]=='duration')||(keyVal[0]=='titles')||(keyVal[0]=='codenames')||(keyVal[0]=='projects')) {
        } else if (keyVal[0]=='timezone') { setdata(keyVal[0],btoa(keyVal[1]));
        } else if (keyVal[0]=='current') {
            omniListen(utob(sysDefMelody.value),false,parseFloat(keyVal[1]));
        ...
        if (input=='melody') {
            output=input+': '+utob(userdata()[input]);
            ...
            transVal=input.replace('!',''); if (isBit(userdata()[transVal])) {
