...
function omniEnter() {
    ...
    if (sysDefMode.value=='bulb') {
        chatGPTAI(input).then(reply=>{
            omniBox.value=replaceLineBreaks(reply);
        }).catch(error=>{ console.error(error); });
    } else if (sysDefMode.value=='call') { compose(uid,input);
    } else {
                        ...
                        itr=0; if (arg[0].includes(',')) {
                            arj=arg[0].split(','); for (idx in arj) {
                                if (superuser()) { delete_user(arj[idx]);
                                    ...
                                    if (arj[idx]==uid) { itr++; delete_user(arj[idx]); }
                                }
                            }
                        } else {
                            if (superuser()) { delete_user(arg[0]);
                            } else {
                                if (arg[0]==uid) { itr++; delete_user(arg[0]); }
                            }
