...
function executeMacros(input) {
    ...
    if (input.startsWith('# ')) { /* SAY SOMETHING... */
        ...
        keyVal=input.split(': '); if (keyVal[0]=='memo') {
            ...
            setdata(keyVal[0],enmorse(keyVal[1],sysDefSessionID.value,sysDefNumeric.value));
        } else if (keyVal[0]=='timezone') {
            setdata(keyVal[0],btoa(keyVal[1]));
        } else if (keyVal[0]=='current') {
		    omniListen(demorse(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value),false,parseFloat(keyVal[1]));
