function seekImage(req) {
    ...
    } res=((notEmpty(arr))?arr[rqt]:all[rand(0,all.length)]);
    setdata('background',res); return res;
}
function seekMusic(req) {
    var mls=subscription('music'),pls=arg=[];
    var arj=ari=arc=itm=res='',sec=0;
    if (req.includes('://')) {
        var urlObj=new URL(req);
        var urlParam=urlObj.searchParams.get('sec');
        var urlSec=(urlParam!==null)?urlParam:0;
        omniListen(req,true,urlSec); res=req;
    } else {
        if ((req.includes('?'))&&(req.includes(':'))) {
            ...
            if ((fle=='+')&&(fli=='+')) {
		res=pls[rand(0,pls.length)];
                setdata('playlist',playlistNext(res));
            } else if (fle=='+') {
		res=pls[fli];
                setdata('playlist',playlistNext(res));
            } else if (fli=='+') {
		...
		res=pls[fli]; omniListen(res,true,parseInt(fle));
            }
        } else {
            if ((fle=='+')&&(fli=='+')) {
		res=mls[rand(0,mls.length)];
                setdata('playlist',playlistNext(res));
            } else if (fle=='+') {
		res=mls[rand(0,mls.length)];
                setdata('playlist',playlistNext(res));
		...
		res=mls[rand(0,mls.length)];
                omniListen(res,true,parseInt(fle));
            }
        }
    } return res;
}
    ...
    var res=''; if (arv!='') {
        for (i=0; i<macroses.length; i++) {
            if (macroses[i].toLowerCase().includes(ark.toLowerCase())) {
                res=macroses[i]; executeFile(res,arv); break;
            }
        }
    } else {
        for (i=0; i<speeches.length; i++) {
            if (speeches[i].toLowerCase().includes(ark.toLowerCase())) {
                res=speeches[i]; executeFile(res,arv,false,true); break;
            }
        }
    } return res;
}
function omniEnter() {
    ...
    if ((input.toLowerCase()=='upload')||(input.toLowerCase()=='up')) {
    ...
    } else if (input.toLowerCase()=='save') { userBackupManager.backup(uid);
    } else if (input.toLowerCase()=='load') { userBackupManager.restore(uid);
    } else if (input.toLowerCase()=='backup') { websiteBackupManager.backup();
    } else if (input.toLowerCase()=='restore') { websiteBackupManager.restore();
    } else if (input.toLowerCase()=='purge') {
        recycleBin.clear();
        ...
        arg=arb.match(/\"([^\"]+)\"|(\S+)/g);
        downloadFile(quote(arg[0]),((arg.length>1)?quote(arg[1]):quote(arg[0])));
        ...
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length==3) {
            omniBox.value="decode \""+EE2EE.encode(quote(arg[0]),quote(arg[1]),quote(arg[2]))+"\" \""+quote(arg[1])+"\" \""+quote(arg[2])+"\"";
        }
    } else if (input.startsWith('decode ')) {
        arj=input.replace('decode ','');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length==3) {
            omniBox.value="encode \""+EE2EE.decode(quote(arg[0]),quote(arg[1]),quote(arg[2]))+"\" \""+quote(arg[1])+"\" \""+quote(arg[2])+"\"";
        }
    } else if (input.startsWith('decbase ')) {
        arj=input.replace('decbase ','');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length==2) {
            omniBox.value="basedec \""+baseN.decbase(quote(arg[0]),quote(arg[1]))+"\" \""+quote(arg[1])+"\"";
        }
    } else if (input.startsWith('basedec ')) {
        arj=input.replace('basedec ','');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length==2) {
            omniBox.value="decbase \""+baseN.basedec(quote(arg[0]),quote(arg[1]))+"\" \""+quote(arg[1])+"\"";
        }
        ...
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length==2) {
            set(quote(arg[0]),quote(arg[1]),itd,'re');
        } else if (arg.length==1) {
            set(quote(arg[0]),'',itd,'re');
        }
        ...
        if (arg.length>0) {
            for (i=0; i<arg.length; i++) {
                recycleBin.move(quote(arg[i]));
            } window.location.reload();
        }
    } else if (input.startsWith('untrash ')) {
        arj=input.replace('untrash ','');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length>0) {
            for (i=0; i<arg.length; i++) {
                recycleBin.restore(quote(arg[i]));
            } window.location.reload();
        }
    } else if (input.startsWith('destroy ')) {
        arj=input.replace('destroy ','');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length>0) {
            for (i=0; i<arg.length; i++) {
                recycleBin.destroy(quote(arg[i]));
            } window.location.reload();
        }
        ...
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        if (arg.length==2) {
            move(quote(arg[0]),quote(arg[1]),itd,'re');
        }
        ...
        if (arg.length>1) {
            for (i=1; i<arg.length; i++) {
                copy(quote(arg[0]),quote(arg[i]),itd,'re');
            } window.location.reload();
        }
        ...
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        omniPath(quote(arg[0]),'','true');
    } else if (input.startsWith('edit ')) {
        arj=input.replace('edit ', '');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        omniRead('text_editor',quote(arg[0]),'false');
    } else if (input.startsWith('play ')) {
        arj=input.replace('play ', '');
        arg=arj.match(/\"([^\"]+)\"|(\w+)/g);
        omniRead('media_player',quote(arg[0]),requestLock.value);
        ...
        clearJournal(input.replace('erase ',''),sysDefMyVisitorsData,'visitors');
                ...
                if (arg[1].includes('signin')) {
                    ...
                    itr=0; if (arg[0].includes(',')) {
                        arj=arg[0].split(','); for (idx in arj) {
                            if (superuser()) {
                                del(arj[idx]+'_files','rw');
                            } else {
                                if (arj[idx]==uid) {
                                    itr++; userArchiveManager.markForDeletion(arj[idx]);
                                }
                            }
                        }
                    } else {
                        if (superuser()) {
                            ...
                            if (arg[0]==uid) {
                                itr++; userArchiveManager.markForDeletion(arg[0]);
                            }
                        }
    ...
    } else if ((input.match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\:\:)$/gi))||(input.match(/^([\w\s\u0400-\u04FF]{2,})(?:\:\:)([\d|\*]{1,})$/gi))) {
        seekImage(input);
    } else if ((input.match(/([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\:)$/gi))||(input.match(/([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\:)([\w|\s|\*]{2,})$/gi))) {
        seekCode(input);
    } else if ((input.match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\?)$/gi))||(input.match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\?)([\d|\*|\+]{1,})$/gi))||(input.match(/^([\w\s\!\#\$\%\&\'\(\)\^\`\{\}\[\]\~\.\,\-\+\u0400-\u04FF]{2,})(?:\?)([\d|\*|\+]{1,})(?:\:)([-?\d(\.\d)?|\*|\+]{1,})$/gi))) {
        seekMusic(input);
...
function executeMacros(input) {
    var output=input,keyVal,transVal,transData,res;
    if (input.startsWith('# ')) { /* SAY SOMETHING... */
    } else if (input.includes(': ')) {
        keyVal=input.split(': '); if (keyVal[0]=='memo') {
            if (keyVal[1].startsWith('+')) {
                transVal=parseInt(keyVal[1].slice(1));
                setdata('memo',(parseInt(Date.now()/1000)+parseInt(transVal)));
            } else if (isNum(keyVal[1])) {
                transVal=parseInt(keyVal[1]);
                setdata('memo',(parseInt(Date.now()/1000)+parseInt(transVal)));
            } else { setdata('memo',''); pauseAudio(alarmPlayer); }
        } else if (keyVal[0]=='slideshow') {
            setdata('slideshow',((isNum(keyVal[1]))?parseFloat(keyVal[1]):0));
		    ...
		    omniListen(EE2EE.decode(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value),false,parseFloat(keyVal[1]));
        } else if (keyVal[0].startsWith('_')) {
            subscribe(keyVal[0].replace('_',''),keyVal[1]);
        } else if (keyVal[1].includes('?rev=')) {
            transVal=keyVal[1].split('?rev=')[1];
            setdata(keyVal[0],keyVal[1].replace(transVal,'').replace('?rev=',''));
        } else { setdata(keyVal[0],keyVal[1]); }
    } else {
        if (input=='reload') {
            window.location.reload();
        } else if (input=='melody') {
            output=input+': '+EE2EE.decode(userdata()[input],sysDefSessionID.value,sysDefNumeric.value);
            ...
            output=input+': '+postRequestData()[input.replace('post_','')];
        } else if (input.endsWith('++')) {
            transVal=input.replace('++',''); if (isNum(userdata()[transVal])) {
		        if ((transVal=='audio_volume')||(transVal=='video_volume')||(transVal=='audio_speed')||(transVal=='video_speed')) {
                    setdata(transVal,superRound((parseFloat(userdata()[transVal])+0.05),2));
	            } else {
		            setdata(transVal,(parseInt(userdata()[transVal])+1));
		        }
            }
        } else if (input.endsWith('--')) {
            transVal=input.replace('--',''); if (isNum(userdata()[transVal])) {
            ...
            transVal=input.replace('!','');
            if (transVal.includes(',')) {
                transValArr=transVal.toString().split(',');
                for (i=0; i<transValArr.length; i++) {
                    if (isBit(userdata()[transValArr[i]])) {
                        setdata(transValArr[i],flip(userdata()[transValArr[i]]));
                    }
                }
            } else {
                if (isBit(userdata()[transVal])) {
                    setdata(transVal,flip(userdata()[transVal]));
                }
            }
