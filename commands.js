    ...
    if (sysDefBackground.value!='') {
        ...
        if ((req.includes('?'))&&(req.split('?').length==2)) {
            rqd=req.split('?'); rqs=rqd[0]; for (idx in all) {
                /*md=(mod[all[idx]]['keywords']!==undefined)?(mod[all[idx]]['keywords']):'';
                mt=(md.toLowerCase().includes(','))?(md.toLowerCase().split(',')):[md.toLowerCase()];*/
...
function executeMacros(input) {
    ...
    if (input.startsWith('# ')) { /* SAY SOMETHING... */
        ...
        } else if ((rep[0]=='playlist')||(rep[0]=='duration')) {
        ...
        } else if (rep[0].startsWith('_')) { subscribe(rep[0].replace('_',''),rep[1]);
        ...
        } else if ((input=='melody')||(input=='timezone')) {
            output=input+': '+atob(userdata()[input]);
        } else if (input=='playlist') {
            output=input+': '+readPlaylist().join(' | ');
