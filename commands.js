function executeMacros(input,index=0,length=1) {
    var output=input,rep,san,atr,atd,atn,atp,atm;
    var plu,cld,np,inc,np1,np2,np3,np4,ml,mr,met=0;
    if ((index==(length-1))&&(input=='_')) { omniBack(sysDefParent.value);
    } else if ((index==(length-1))&&(input == '\\=')) {
        output='\\='+dtw(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value);
    } else if ((index==(length-1))&&(input=='&')) { unbind(sysDefSessionID.value);
    } else if ((index==(length-1))&&(input=='$')) { unequip(sysDefSessionID.value);
    } else if (input.startsWith('# ')) { /* SAY SOMETHING... */
    } else if (input.startsWith('>')) { rep=input.replace('>','');
        if ((rep=='auto')||(rep=='manual')) { administer('auto',rep,'automator');
        } else if ((rep=='bind')||(rep=='call')) { administer(rep,rep,rep+'ing');
        } else if (rep=='friend') { administer(rep,rep,rep+'ship');
        } else if (rep=='tool') { administer(rep,rep,rep+'box');
        } else { administer('powers',rep,'dominion'); }
        ...
        rep=input.replace('~',''),met=0; if (rep.includes(',')) {
            san=rep.split(','); for (ir in san) {
                if (superuser()) { delete_user(san[ir]);
                } else { if (san[ir]==sysDefSessionID.value) { met++; delete_user(san[ir]); }}
            }
        } else {
            if (superuser()) { delete_user(rep);
            } else { if (rep==sysDefSessionID.value) { met++; delete_user(rep); }}
        } if (met>0) { omniAuthRequest('signout','',''); }
    ...
    } else if ((index==(length-1))&&(input.startsWith('('))&&(input.endsWith(')'))) {
        call(sysDefSessionID.value,input.replace('(','').replace(')',''));
    } else if ((index==(length-1))&&(input.startsWith('_'))) { omniGo(input.replace('_',''));
    } else if ((index==(length-1))&&(input.startsWith('='))) {
        window.location.href=input.replace('=','');
    } else if ((index==(length-1))&&(input.startsWith('+'))) {
        if (input.replace('+','').includes('h')) {
            plu=parseInt(input.replace('+','').replace('h',''))*3600;
            setdata('memo',(Math.round(Date.now()/1000)+parseInt(plu)));
        } else if (input.replace('+','').includes('min')) {
            plu=parseInt(input.replace('+','').replace('min',''))*60;
            setdata('memo',(Math.round(Date.now()/1000)+parseInt(plu)));
        } else if (input.replace('+','').includes('m')) {
            plu=parseInt(input.replace('+','').replace('m',''))*60;
            setdata('memo',(Math.round(Date.now()/1000)+parseInt(plu)));
    ...
    } else if ((index==(length-1))&&(isInt(input))) { audioPosition(input);
    } else {
...
function pipeExec(input) {
    if (input.includes('/')) {
        var exr=(input.endsWith('/')),pipes=input.split('/');
...
function omniEnter() {
    ...
    if (sysDefChat.value!=0) { compose(input); } else {
        ...
        } else if (input=='clear') { clearJournal('',sysDefMsgData,'msgbox');
        } else if (input=='erase') { clearJournal(-1,sysDefBookKeep,'book');
        ...
        } else if ((input.startsWith('/'))||(input.startsWith('\\'))) { pipeExec(input);
