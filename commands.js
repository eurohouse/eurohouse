...
function omniEnter() {
    ...
    if (sysDefMode.value=='mail') { compose(input);
        ...
        if ((input=='reload')||(input=='refresh')) { window.location.reload();
        ...
        } else if ((input=='save')||(input=='backup')) {
            ...
            arb=input.replace('\\=',''); if (arb.includes('//')) {
                ...
                arg=arb.split('/'); if (arg.length==3) {
                    omniListen('https://github.com/'+arg[0]+'/'+arg[1]+'/blob/main/'+arg[2]+'?raw=true',true);
                }
            }
        } else if (input.startsWith('./')) {
            ...
            arb=input.replace('~',''),itr=0; if (arb.includes(',')) {
                arj=arb.split(','); for (idx in arj) {
                    if (superuser()) { delete_user(arj[idx]);
                        ...
                        if (arj[idx]==sysDefSessionID.value) { itr++; delete_user(arj[idx]); }
                    }
                }
            } else {
                if (superuser()) { delete_user(arb);
                } else {
                    if (arb==sysDefSessionID.value) { itr++; delete_user(arb); }
                }
