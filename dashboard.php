<div class='topBarItem'>
    ...
    } if (requestMode.value=='accessibility') { pressedKeyCode.innerText=event.keyCode; }">
...
<div class='topBarItem'>
    <p align='center' class='block'>
    <?php if (isAuth()) { ?>
        <input type="image" onmouseover="soundButton();" id="buttonChat" class="power" onclick="setdata('chat',flip(sysDefChat.value)); omniBox.focus();" src="<?=$prefix.(($session['chat']!=0)?'book.png':'bash.png');?>" title="<?=term('Chat Console Mode',$settings['vocabulary'],$session['units']);?>">
        <input type='text' id="omniBox" style="width:59%;" placeholder="<?=term('Type expression and press ENTER',$settings['vocabulary'],$session['units']);?>" value="" onkeydown="if (event.keyCode==13) { omniEnter(); } else if (event.keyCode==27) {
            document.getElementById('omniBox').value='';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
        } else if (event.keyCode==113) {
            setdata('chat',flip(sysDefChat.value)); omniBox.focus();
        } if (requestMode.value=='accessibility') { pressedKeyCode.innerText=event.keyCode; }" oninput="handleInput(this.value,true);">
...
<div class='topBarItem'>
    <p align='center' class='block'>
    <input type="image" onmouseover="soundButton();" id="buttonTime" class="power" onclick="setdata('benchmark',nextImage('0;2',sysDefBenchmark.value));" src="<?=$prefix.'time.png';?>" title="<?=term('Go Through Daily Wallpaper Series',$settings['vocabulary'],$session['units']);?>">
...
<div class='topBarItem'>
    ...
    <input type="image" onmouseover="soundButton();" id="buttonPitched" class="power" onclick="setdata('pitch_lock',flip(sysDefPitchLock.value));" src="<?=$prefix.(($session['pitch_lock']!=0)?'midi.png':'volume.png');?>" title="<?=term('Audio Preserve Pitch',$settings['vocabulary'],$session['units']);?>">
...
<div class='topBarItem'>
    ...
    <input type='button' id="showUsInfoBond" style="width:24%;" value="<?=$sessionID;?>"><input type="image" onmouseover="soundButton();" id="buttonSpectate" class="power" onclick="setdata('spectate',flip(sysDefSpectate.value));" src="<?=$prefix.'unpower.png';?>" title="<?=term('Spectate Mode',$settings['vocabulary'],$session['units']);?>">
    </p>
</div>
<div class='topBarItem'>
    <p align='center' class='block'>
