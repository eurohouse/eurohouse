...
<div class='topBarItem'>
    ...
    <?php if (isAuthorized()) { ?>
        ...
        <input type='text' id="omniBox" style="width:58%;" placeholder="<?=term('Type command or expression and press ENTER',$settings,$session);?>" value="" onkeydown="if (event.keyCode==13) { omniEnter(omniBox.value);
        ...
        <input type="image" onmouseover="soundButton();" id="buttonEnter" class="power" onclick="omniEnter(omniBox.value);" src="<?=$prefix[3].'return.png';?>" title="<?=term('Execute Command',$settings,$session);?>">
...
<div class='topBarItem'>
    ...
    <input type="image" onmouseover="soundButton();" id="buttonReticle" class="power" onclick="var uli=(sysDefUsersList.value).split(','),bdi=jsonarr(sysDefBindData.value); delete uli[sysDefSessionID.value]; if (bdi[sysDefSessionID.value]!=sysDefSessionID.value) { bind(sysDefSessionID.value,sysDefSessionID.value); } else { bind(sysDefSessionID.value,uli[rand(0,(uli.length))]); } dominate(sysDefSessionID.value,jsonarr(sysDefBindData.value)[sysDefSessionID.value],jsonarr(sysDefToolData.value)[sysDefSessionID.value]); playAudio(hitPlayer,sysDefHitSound.value);" src="<?=$prefix[5].$session['reticle'].'.png';?>" title="<?=term('Hit Another User',$settings,$session);?>">
