...
<div class='topBarItem'>
    ...
    <input id="showUserAvatarBadgeTop" type='image' onmouseover="soundButton();" class="power" src="<?=$prefix[0].$session['avatar'].'.png';?>" title="<?=term('User Avatar',$settings,$session);?>" onclick="soundClick(); setdata('face',flip(sysDefFace.value));">
    ...
    <input type="image" id="buttonPlay" onmouseover="soundButton();" class="power" onclick="soundClick(); if (sysDefPlaying.value!=0) { omniPause(); } else { omniListen(EE2EE.decode(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value),false,sysDefCurrent.value); } setdata('playing',flip(sysDefPlaying.value));" src="<?=$prefix[3].(($session['playing']!=0)?'pause.png':'play.png');?>" title="<?=term('Audio Play/Pause',$settings,$session);?>">
