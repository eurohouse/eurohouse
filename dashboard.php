...
<div class='topBarItem'>
    ...
    <?php if (isAuthorized()) { ?>
        ...
        <input type='text' id="omniBox" style="width:60%;" placeholder="<?=term('',$settings['locale']['cli'],$session['units'],$session['mode']);?>" value="" onkeydown="if (event.keyCode==13) { omniEnter(); } else if (event.keyCode==27) { omniBox.value='';
        } else if (event.keyCode==8) { handleInput(this.value);
        } else if (event.keyCode==46) { handleInput(this.value);
...
<div class='topBarItem'>
    ...
    <?php } ?></select>
    <input type="image" onmouseover="soundButton();" id="buttonReqLock" class="power" onclick="if (requestLock.value=='true') { omniLock('false'); } else { omniLock('true'); }" src="<?=$prefix[3].(($request['lock']=='true')?'collapse.png':'expand.png');?>" title="<?=term('Expand/Collapse Enhanced View',$settings['vocabulary'],$session['units']);?>">
