...
<div class='topBarItem'>
    <p align='center' class='block'>
    <?php if (isAuth()) { ?>
        <input type="image" onmouseover="soundButton();" id="buttonBackup" class="power" onclick="var ss=sysDefSessionID.value; if (sysDefSessBackup.value!=0) { copy(ss+'_backup.json',ss+'_session.json',true,1); del(ss+'_backup.json',true); readFile(ss+'_session.json'); var rf=jsonarr(localStorage.getItem('user_content')); omniListen(dtw(rf['melody'],ss,rf['numeric']),false,parseInt(rf['current'])); } else { copy(ss+'_session.json',ss+'_backup.json',true,1); }" src="<?=$prefix.'save.png';?>" title="<?=term('Save/Load User Session Backup',$settings['vocabulary'],$session['units']);?>">
