...
<div class='topBarItem'>
    ...
    <?php if (isAuth()) { ?>
        ...
        <input type="image" onmouseover="soundButton();" id="buttonSearch" class="power" onclick="setdata('mode','search'); omniBox.focus();" src="<?=$prefix.'search.png';?>" title="<?=term('File Search Mode',$settings['vocabulary'],$session['units']);?>">
        <input type='text' id="omniBox" style="width:49%;" placeholder="<?=term('Type expression and press ENTER',$settings['vocabulary'],$session['units']);?>" value="" onkeydown="if (event.keyCode==13) { omniEnter(); } else if (event.keyCode==27) {
