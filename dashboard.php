<div class='topBarItem'>
    ...
    <input type="image" onmouseover="soundButton();" id="buttonRandomMusic" class="power" onclick="var objCmdTxt=(authstate())?omniBox:omniBoxAuthLogin; seekMusic(objCmdTxt.value);" src="<?=$prefix[3].'music.png';?>" title="<?=term('Random Track',$settings,$session);?>">
    <input type="image" onmouseover="soundButton();" id="buttonRandomBanner" class="power" onclick="var objCmdTxt=(authstate())?omniBox:omniBoxAuthLogin; if (sysDefBanner.value!='') { setdata('banner',''); } else { seekBanner(objCmdTxt.value); }" src="<?=$prefix[3].(($session['banner']!='')?'paint.png':'image.png');?>" title="<?=term('Random Banner',$settings,$session);?>">
    <select id="ErotoOlympus" style="width:34%;" onchange="setdata('background',ErotoOlympus.options[ErotoOlympus.selectedIndex].id);"><?php foreach ($userLocks['background'] as $key=>$val) { ?>
...
<div class='topBarItem'>
    ...
    <?php } else { ?>
        ...
        <input type='text' id="omniBoxAuthLogin" style="width:31%;" placeholder="<?=term('Username',$settings,$session);?>" value="" onkeydown="if (event.keyCode==13) {
            ...
            seekMusic(omniBoxAuthLogin.value);
        } else if (event.keyCode==118) {
            seekBanner(omniBoxAuthLogin.value);
        ...
        <input type='password' id="omniBoxAuthPass" style="width:31%;" placeholder="<?=term('Password',$settings,$session);?>" value="" onkeydown="if (event.keyCode==13) {
            ...
            seekMusic(omniBoxAuthPass.value);
        } else if (event.keyCode==118) {
            seekBanner(omniBoxAuthPass.value);
        } keyPressed();" oninput="handleInput(this.value,true);">
        ...
        <input type="image" onmouseover="soundButton();" id="buttonBackspace" class="power" onclick="omniBoxAuthPass.value=''; omniBoxAuthLogin.value='';
        omniBoxAuthLogin.focus();" src="<?=$prefix[3].'backspace.png';?>" title="<?=term('Clear Sign In Form',$settings,$session);?>">
        <input type="image" onmouseover="soundButton();" id="buttonUserStatus" class="power" onclick="if (authstate()) { omniAuthRequest('signout','',''); } else { omniAuthRequest('signin',omniBoxAuthLogin.value,CryptoJS.SHA256(omniBoxAuthPass.value).toString()); }" src="<?=$prefix[3].'forward.png';?>" title="<?=term('Sign In/Out',$settings,$session);?>">
    <?php } ?>
    </p>
</div>
<div class='topBarItem'>
    ...
    <input type='button' id="currentTime" style="width:32%;" onclick="setdata('timedisp',flip(sysDefTimedisp.value));" value="<?=date(($session['timedisp']!=0)?$session['date_format']:$session['time_format']);?>">
...
<div class='topBarItem'>
    ...
    <?php } ?></select>
    <select id="setTheme" style="width:36%;" onchange="setdata('theme',setTheme.options[setTheme.selectedIndex].id); window.location.reload();">
