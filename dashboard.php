...
<div class='topBarItem'>
    ...
    <?php } else { ?>
        <input type="image" onmouseover="soundButton();" id="buttonSuggest" class="power" onclick="var uli=(sysDefUsersList.value).split(',');omniBoxAuthLogin.value=uli[rand(0,(uli.length))];omniBoxAuthPass.value='';omniBoxAuthPass.focus();" src="<?=$prefix.'user.png';?>" title="<?=term('Suggest Username',$settings['vocabulary'],$session['units']);?>">
...
<div class='topBarItem'>
    ...
    <input type='button' id="alarmTime" style="width:30%;" onclick="setdata('memo',''); pauseAudio(alarmPlayer);" value="00:00"><input type="image" id="buttonPlay" onmouseover="soundButton();" class="power" onclick="if (sysDefPlaying.value==1) { omniPause(); } else { omniListen(demorse(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value)); }" src="<?=$prefix.(($session['playing']!=0)?'pause.png':'play.png');?>" title="<?=term('Audio Play/Pause',$settings['vocabulary'],$session['units']);?>">
...
<div class='topBarItem'>
    ...
    <input type="image" onmouseover="soundButton();" id="buttonBroke" class="power" onclick="var uli=(sysDefUsersList.value).split(','),bdi=strarr(sysDefBindData.value,';',':'); delete uli[sysDefSessionID.value]; if (bdi[sysDefSessionID.value]!=sysDefSessionID.value) { unbind(sysDefSessionID.value); } else { bind(sysDefSessionID.value,uli[rand(0,(uli.length))]); }" src="<?=$prefix.'chain.png';?>" title="<?=term('Bind/Unbind Another User',$settings['vocabulary'],$session['units']);?>">
    ...
    <input type="image" onmouseover="soundButton();" id="buttonSpectate" class="power" onclick="setdata('spectate',flip(sysDefSpectate.value));" src="<?=$prefix.'camera.png';?>" title="<?=term('Spectate Mode',$settings['vocabulary'],$session['units']);?>">
    </p>
</div>
<div class='topBarItem'>
    <p align='center' class='block'>
