<div class='topBarItem'>
    ...
    <input type="number" min='-11' max='12' step='1' id="setTimeZone" style="width:8%;" value="<?=$session['timezone'];?>" oninput="setdata('timezone', pad(setTimeZone.value, 2)); handleInput(this.value, true);" onkeydown="if (event.keyCode == 27) {
...
<div class='topBarItem'>
    ...
    <?php if (isAuth()) { ?>
        ...
        <input type='text' id="omniBox" style="width:58%;" placeholder="<?=term('Type expression and press ENTER', $settings['vocabulary'], $session['units']);?>" value="" onkeydown="
        if (event.keyCode == 13) {
            omniEnter();
...
<div class='topBarItem'>
    ...
    <input type="image" onmouseover="soundButton();" id="buttonMaximize" class="power" onclick="setdata('apps', flip(sysDefApps.value)); window.location.reload();" src="<?=$prefix.(($session['apps'] != 0)?'restore.png':'maximize.png').$suffix;?>">
    <input type="image" onmouseover="soundButton();" id="buttonMenuStyle" class="power" onclick="setdata('icons', flip(sysDefIcons.value)); window.location.reload();" src="<?=$prefix.(($session['icons'] != 0)?'menu.png':'list.png').$suffix;?>">
