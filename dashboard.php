<div class='topBarItem'>
    ...
    <input type="image" onmouseover="soundButton();" id="buttonMusic" class="power" onclick="soundClick(); var objCmdTxt=(authstate())?omniBox:omniBoxAuthLogin; seekMusic(objCmdTxt.value); objCmdTxt.focus();" src="<?=$prefix[3].'music.png';?>" title="<?=term('Random Track',$settings,$session);?>">
    <input type="image" onmouseover="soundButton();" id="buttonImage" class="power" onclick="soundClick(); var objCmdTxt=(authstate())?omniBox:omniBoxAuthLogin; if (sysDefBackground.value!='') { setdata('background',''); } else { seekImage(objCmdTxt.value); } objCmdTxt.focus();" src="<?=$prefix[3].(($session['background']!='')?'paint.png':'image.png');?>" title="<?=term('Random Background',$settings,$session);?>">
    <select id="wallpaperCollection" style="width:53%;" onchange="setdata('background_buffer',this.options[this.selectedIndex].id);">
...
<div class='topBarItem'>
    ...
    <?php if (isAuthorized()) { ?>
        ...
        <input type='text' id="omniBox" style="width:62%;" placeholder="<?=term("Type anything and press ENTER",$settings,$session);?>" value="" onkeydown="if (event.keyCode==13) { omniEnter();
...
<div class='topBarItem'>
    <p align='center' class='block'>
    <input type="image" onmouseover="soundButton();" id="buttonSlideshow" class="power" onclick="soundClick(); setdata('slideshow',nextImage('0;5',sysDefSlideshow.value));" src="<?=$prefix[3].(($session['slideshow'])?'speed.png':'time.png');?>" title="<?=term('Toggle Slideshow/Dynamic Wallpaper',$settings,$session);?>">
    <input type='button' id="alarmTime" style="width:62%;" onclick="soundClick(); setdata('memo',''); pauseAudio(alarmPlayer);" value="00:00">
    <input type="image" id="buttonPlay" onmouseover="soundButton();" class="power" onclick="soundClick(); if (sysDefPlaying.value!=0) { omniPause(); } else { omniListen(utob(sysDefMelody.value),false,sysDefCurrent.value); } setdata('playing',flip(sysDefPlaying.value));" src="<?=$prefix[3].(($session['playing']!=0)?'pause.png':'play.png');?>" title="<?=term('Audio Play/Pause',$settings,$session);?>">
...
<div class='topBarItem'>
    ...
    <input type='button' id="usernameBanner" style="width:27%;" onclick="soundClick(); clip(this.value);" value="<?=$sessionID;?>">
