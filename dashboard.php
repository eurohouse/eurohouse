...
<div class='topBarItem'>
    ...
    <?php } else { ?>
        <input type="image" onmouseover="soundButton();" id="buttonSuggest" class="power" onclick="var uli=(sysDefUsersList.value).split(',');omniBoxAuthLogin.value=uli[rand(0,(uli.length))];" src="<?=$prefix.((isInBackup($sessionID))?'open.png':'save.png');?>" title="<?=term('Save/Load User Session Backup',$settings['vocabulary'],$session['units']);?>">
