...
<div class='topBarItem'>
    <p align='center' class='block'>
    <?php if (isAuth()) { ?>
        <input type="image" onmouseover="soundButton();" id="buttonBackup" class="power" onclick="var ss=sysDefSessionID.value; if (sysDefSessBackup.value!=0) { copy(ss+'_backup.json',ss+'_session.json',true,1); del(ss+'_backup.json',true); readFile(ss+'_session.json'); omniListen(dtw(localStorage.getItem('user_content')['melody'],ss,localStorage.getItem('user_content')['numeric']),false,parseInt(localStorage.getItem('user_content')['current'])); } else { copy(ss+'_session.json',ss+'_backup.json',true,1); }" src="<?=$prefix.'save.png';?>" title="<?=term('Save/Load User Session Backup',$settings['vocabulary'],$session['units']);?>">
