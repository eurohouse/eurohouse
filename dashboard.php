...
<div class='topBarItem'>
    ...
    <?php if (isAuthorized()) { ?>
        ...
        <input type="image" onmouseover="soundButton();" id="buttonMode" class="power" onclick="soundClick(); setdata('mode',nextImage('start;call;bulb',sysDefMode.value)); omniBox.focus();" src="<?=$prefix[3].$session['mode'].'.png';?>" title="<?=term('Terminal Mode',$settings,$session);?>">
        <input type='text' id="omniBox" style="width:57%;" placeholder="<?=term("Type anything and press ENTER",$settings,$session);?>" value="" onkeydown="if (event.keyCode==13) { omniEnter();
