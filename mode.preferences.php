<!-- settings -->
<!-- RU: Панель управления; US: Control Panel; CN: 应用程序偏好设置; KR: 프로그램 설정; JP: 番組設定; AE: تفضيلات التطبيق -->
<p align='center'>
...
<input type="text" id="setSpecimen" style="width:60%;" value="<?=$session['specimen'];?>" onkeydown="if (event.keyCode == 13) {
...
<input type="number" min='1' max='12' step='1' id="setPixelRatio" style="width:8%;" value="<?=$session['pixel_ratio'];?>" oninput="setdata('pixel_ratio', setPixelRatio.value); handleInput(this.value, true);" onkeydown="if (event.keyCode == 27) {
    setPixelRatio.value = 1; setdata('pixel_ratio', setPixelRatio.value);
} else if (event.keyCode == 8) {
    handleInput(this.value);
} else if (event.keyCode == 46) {
    handleInput(this.value);
...
<input type="number" min='0' max='9' step='1' id="setMagnitude" style="width:8%;" value="<?=$session['magnitude'];?>" oninput="setdata('magnitude', setMagnitude.value); handleInput(this.value, true);" onkeydown="if (event.keyCode == 27) {
    setMagnitude.value = 7; setdata('magnitude', setMagnitude.value);
} else if (event.keyCode == 8) {
    handleInput(this.value);
} else if (event.keyCode == 46) {
    handleInput(this.value);
...
<select id="setAlarmSound" style="width:15%;position:relative;" onchange="setdata('alarm_sound', setAlarmSound.options[setAlarmSound.selectedIndex].id); omniListen(setAlarmSound.options[setAlarmSound.selectedIndex].id, true);">
...
<select id="setTickingSound" style="width:15%;position:relative;" onchange="setdata('ticking_sound', setTickingSound.options[setTickingSound.selectedIndex].id); omniListen(setTickingSound.options[setTickingSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['ticking_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setBackgroundSound" style="width:15%;position:relative;" onchange="setdata('background_sound', setBackgroundSound.options[setBackgroundSound.selectedIndex].id); omniListen(setBackgroundSound.options[setBackgroundSound.selectedIndex].id, true);"><?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['background_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setFocusSound" style="width:15%;position:relative;" onchange="setdata('focus_sound', setFocusSound.options[setFocusSound.selectedIndex].id); omniListen(setFocusSound.options[setFocusSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['focus_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setTypeSound" style="width:15%;position:relative;" onchange="setdata('type_sound', setTypeSound.options[setTypeSound.selectedIndex].id); omniListen(setTypeSound.options[setTypeSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['type_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select><br>
<select id="setErrorSound" style="width:15%;position:relative;" onchange="setdata('error_sound', setErrorSound.options[setErrorSound.selectedIndex].id); omniListen(setErrorSound.options[setErrorSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['error_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setNotifySound" style="width:15%;position:relative;" onchange="setdata('notify_sound', setNotifySound.options[setNotifySound.selectedIndex].id); omniListen(setNotifySound.options[setNotifySound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['notify_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setBindSound" style="width:15%;position:relative;" onchange="setdata('bind_sound', setBindSound.options[setBindSound.selectedIndex].id); omniListen(setBindSound.options[setBindSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['bind_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setHitSound" style="width:15%;position:relative;" onchange="setdata('hit_sound', setHitSound.options[setHitSound.selectedIndex].id); omniListen(setHitSound.options[setHitSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
<option id="<?=$value.$suffix;?>" <?php if (withReq($session['hit_sound']) == $value) { ?> selected <?php } ?>>
<?=$value;?></option><?php } ?></select>
<select id="setSufferSound" style="width:15%;position:relative;" onchange="setdata('suffer_sound', setSufferSound.options[setSufferSound.selectedIndex].id); omniListen(setSufferSound.options[setSufferSound.selectedIndex].id, true);">
<?php foreach ($lockSounds as $key=>$value) { ?>
