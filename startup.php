...
function world_clock() {
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#buttonMenu').attr('src',sysDefPrefix.value+((sysDefIcons.value!=0)?'menu.png':'list.png'));
            $('#buttonUpdate').attr('src',sysDefPrefix.value+'world.png');
            $('#buttonCommand').attr('src',sysDefPrefix.value+'start.png');
            $('#buttonCompose').attr('src',sysDefPrefix.value+'keyboard.png');
            $('#buttonSuggest').attr('src',sysDefPrefix.value+'dice.png');
            $('#buttonMusic').attr('src',sysDefPrefix.value+'music.png');
            $('#buttonBanner').attr('src',sysDefPrefix.value+((sysDefBanner.value!='')?'paint.png':'image.png'));
            $('#buttonLock').attr('src',sysDefPrefix.value+((requestLock.value!='false')?'restore.png':'maximize.png'));
            $('#buttonReload').attr('src',sysDefPrefix.value+((sysDefReload.value!=0)?'bluetooth.png':'radio.png'));
            $('#buttonAutoplay').attr('src',sysDefPrefix.value+((sysDefAutoplay.value!=0)?'autopause.png':'autoplay.png'));
            $('#buttonIndex').attr('src',sysDefPrefix.value+((sysDefSongIndex.value=='random')?'shuffle.png':'update.png'));
            ...
            if (authstate()) {
                ...
                for (iu in upn) {
                    arl+="<a href='javascript:setdata(%22playlist%22,arrangeMenu(sysDefPlaylist.value,enmorse(upn[iu],sysDefSessionID.value,sysDefNumeric.value),' | '));omniListen(%22"+rfc3986(upn[iu])+"%22,true);'>"+(parseInt(iu)+1)+'. '+upn[iu]+"</a><br>";
