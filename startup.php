...
function databox() {
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#sysDefGovtData').val(pager(data,10));
            $('#sysDefLockData').val(pager(data,11));
            $('#sysDefCodexBox').val(pager(data,12).split("\\\\")[0]);
            $('#sysDefSpeechBox').val(pager(data,12).split("\\\\")[1]);
            $('#sysDefUsersList').val(pager(data,13));
            $('#sysDefHoursNow').val(pager(data,14));
            $('#sysDefHoursActive').val(pager(data,15));
            $('#sysDefAvatarsNow').val(pager(data,16));
            $('#sysDefMetaData').val(pager(data,17));
            $('#sysDefTutorData').val(pager(data,18));
...
function world_clock() {
    $.ajax({
        ...
        success: function(data) {
            ...
            if (requestMode.value=='messenger') { msgBox.innerHTML='<p>'+jsonHTML(sysDefMsgData.value,sysDefFind.value)+'</p>'; } if (requestMode.value=='news_feed') { newsBox.innerHTML='<p>'+jsonNews()+'</p>'; }
            ...
            if (requestMode.value=='bookkeeping') {
                ...
                govt_disp.innerHTML='<table style="width:100%;position:relative;"><thead><th style="width:25%;">Plaintiff</th><th style="width:25%;">Defendant</th><th style="width:25%;">Claims</th><th style="width:25%;">Status</th></thead><tbody>'+jsonDocs(sysDefGovtData.value)+'</tbody></table>';
