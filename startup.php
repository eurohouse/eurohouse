...
function world_clock() {
    $.ajax({
        ...
        success: function(data) {
            ...
            showUsCurrentMusic.innerText=(sysDefCipher.value!=0)?sysDefMelody.value:demorse(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value);
	    ...
	    $('#buttonTrash').attr('src',sysDefPrefix.value+'trash.png');
            ...
            document.querySelector(':root').style.setProperty('--melody-marquee-animation','scrollMarquee '+sysDefMelodyMarqueeSpeed.value+'s linear infinite');
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            faviconList.forEach(function(element) {
                ...
                if ($request['mode']=='user_profile') { ?>
                    $('#showUserHead').text(pager(data,0).toUpperCase());
                    $('#showUserBody').html(pager(data,1));
                    $('#showUserAvatar').attr('src',pager(data,3));
                <?php } elseif ($request['mode']=='preferences') { ?>
                    $('#showUserAvatarBadge').attr('src',pager(data,3));
		    $('#showUserTypeBadge').attr('src',sysDefPrefix.value+((sysDefType.value=='entity')?'home.png':'user.png'));
                <?php } ?> $('#showUsText').html(pager(data,6));
            <?php } else { ?>
                $('#showArticleHead').text(pager(data,5).toUpperCase());
                $('#showArticleBody').html(pager(data,6));
                $('#showArticleLink').text(pager(data,7));
                $('#showArticleLink').attr('href',pager(data,7));
                $('#showArticleAvatar').attr('src',pager(data,4));
            <?php } ?>
        }
    });
}
