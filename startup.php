...
function world_clock() {
    $.ajax({
        ...
        success: function(data) {
            ...
            showUsCurrentMusic.innerText=(sysDefCipher.value!=0)?sysDefMelody.value:('"'+demorse(sysDefMelody.value,sysDefSessionID.value,sysDefNumeric.value)+'" <'+hhmmss(sysDefCurrent.value,true)+'/'+hhmmss(sysDefDuration.value,true)+'>');
...
function wallpaper_engine() {
    $.ajax({
        ...
        success: function(data) {
	        ...
	        element.setAttribute('href',pager(data,3));
            }); $('#showUserAvatarBadge').attr('src',pager(data,3));
            var headPageNum=(sysDefFace.value!=0)?0:5;
			var bodyPageNum=(sysDefFace.value!=0)?1:6;
			var avatarPageNum=(sysDefFace.value!=0)?3:4;
